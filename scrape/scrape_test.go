...
func TestScrapeLoopDiscardDuplicateLabels(t *testing.T) {
	...
	require.NoError(t, err)
	series := q.Select(context.Background(), false, nil, labels.MustNewMatcher(labels.MatchRegexp, "__name__", ".*"))
	...
	q, err = s.Querier(time.Time{}.UnixNano(), 0)
	require.NoError(t, err)
	series = q.Select(context.Background(), false, nil, labels.MustNewMatcher(labels.MatchEqual, "le", "500"))
