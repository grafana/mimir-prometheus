...
func TestOTLPLabelSanitization(t *testing.T) {
	t.Run("defaults to true", func(t *testing.T) {
		conf, err := LoadFile(filepath.Join("testdata", "otlp_sanitize_underscores_defaults.good.yml"), false, promslog.NewNopLogger())
		require.NoError(t, err)

		// Test that default values are true
		require.True(t, conf.OTLPConfig.LabelNameUnderscoreLabelSanitization)
		require.True(t, conf.OTLPConfig.LabelNamePreserveMultipleUnderscores)
	})

	t.Run("explicit enabled", func(t *testing.T) {
		conf, err := LoadFile(filepath.Join("testdata", "otlp_sanitize_underscores_enabled.good.yml"), false, promslog.NewNopLogger())
		require.NoError(t, err)
		...
		require.True(t, got.OTLPConfig.LabelNameUnderscoreLabelSanitization)
		require.True(t, got.OTLPConfig.LabelNamePreserveMultipleUnderscores)
	})

	t.Run("explicit disabled", func(t *testing.T) {
		conf, err := LoadFile(filepath.Join("testdata", "otlp_sanitize_underscores_disabled.good.yml"), false, promslog.NewNopLogger())
		require.NoError(t, err)

		// When explicitly set to false, they should be false
		require.False(t, conf.OTLPConfig.LabelNameUnderscoreLabelSanitization)
