...
type MemPostings struct {
	...
	ordered     bool
	infoMetrics map[storage.SeriesRef]labels.Labels
}
...
func NewMemPostings() *MemPostings {
	return &MemPostings{
		...
		ordered:     true,
		infoMetrics: make(map[storage.SeriesRef]labels.Labels, 512),
	}
}
...
func (p *MemPostings) AddInfoMetric(id storage.SeriesRef, lset labels.Labels, t int64, identifyingLabels []int) {
	...
	lset.Range(func(l labels.Label) {
		...
		case l.Name == labels.MetricName:
			// Ignore.
		case j >= len(identifyingLabels):
			// There are no more identifying labels => data label.
			dlsB.Add(l.Name, l.Value)
		case i == identifyingLabels[j]:
			j++
	...
	ils := ilsB.Labels()
	existingILs, exists := p.infoMetrics[id]
	if exists {
		if !labels.Equal(ils, existingILs) {
			fmt.Printf("MemPostings: Info metric series %d is already in p.infoMetrics but with different identifying labels, t: %d, identifying labels: %#v\n", id, t, ilsB)
		}
		return
	}

	fmt.Printf("MemPostings: Adding new info metric entry for series %d, t: %d, identifying labels: %#v\n", id, t, ilsB)
	p.infoMetrics[id] = ils
}
...
func (p *MemPostings) InfoMetricDataLabels(ctx context.Context, lbls labels.Labels, t int64, latestTimestamp func(storage.SeriesRef) (int64, labels.Labels, error), matchers ...*labels.Matcher) (labels.Labels, error) {
	p.mtx.RLock()
	defer p.mtx.RUnlock()

	var infoMetrics map[storage.SeriesRef]labels.Labels
	if len(matchers) == 0 {
	...
	lblMap := lbls.Map()
	infoMetricEntries := map[string]infoMetricEntry{}
	// Find info metrics with matching identifying labels.
	for sr, ils := range infoMetrics {
		ilsMap := map[string]string{}
		ils.Range(func(l labels.Label) {
			ilsMap[l.Name] = l.Value
		})

		isMatch := true
		for n, v := range ilsMap {
			matchVal, exists := lblMap[n]
			if !exists || matchVal != v {
				isMatch = false
		...
		if !isMatch {
			// The info metric's identifying label set doesn't match.
			continue
		}

		// The info metric's identifying label set matches, try to add its data labels.
		fmt.Printf("MemPostings: This info metric matches\n")

		st, lbls, err := latestTimestamp(sr)
		if err != nil {
			return labels.Labels{}, err
		}

		dlsB := labels.NewScratchBuilder(lbls.Len() - len(ilsMap))
		var name string
		lbls.Range(func(l labels.Label) {
			...
			if _, exists := ilsMap[l.Name]; !exists {
				// Neither identifying nor the name label => data label
				dlsB.Add(l.Name, l.Value)
			}
		})
		if name == "" {
			panic("unable to find metric name")
		}

		ie, exists := infoMetricEntries[name]
		if exists && ie.t >= st {
			// Existing one is up to date.
			continue
		}

		infoMetricEntries[name] = infoMetricEntry{
			t:          st,
	...
	for _, ie := range infoMetricEntries {
		ie.dataLabels.Range(func(l labels.Label) {
			if _, exists := dls[l.Name]; !exists {
				dls[l.Name] = l.Value
			}
		})
	}
	dlb := labels.NewScratchBuilder(len(dls))
	for n, v := range dls {
		dlb.Add(n, v)
	}
...
// infoMetricEntry represents an info metric's data labels at a certain timestamp.
// This is used to discern between conflicting versions of the same info metric name,
// e.g. target_info, where we choose based on timestamp.
