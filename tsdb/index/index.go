...
func (dec *Decoder) InfoMetricDataLabels(ctx context.Context, b []byte, lbls labels.Labels, t int64, matchers ...*labels.Matcher) (labels.Labels, error) {
	...
	k := d.Uvarint()
	if d.Err() != nil {
		return nil, fmt.Errorf("read series label count: %w", d.Err())
	}
	for i := 0; i < k; i++ {
	...
	k = d.Uvarint()
	if d.Err() != nil {
		return nil, fmt.Errorf("read series chunks count: %w", d.Err())
	}
	if k == 0 {
		return nil, nil
	}
	...
	ref0 := int64(d.Uvarint64())
	if d.Err() != nil {
		return nil, fmt.Errorf("read series chunk base times: %w", d.Err())
	}
	...
	*/
	...
	for i := 1; i < k; i++ {
		...
		if d.Err() != nil {
			return nil, fmt.Errorf("read meta for chunk %d: %w", i, d.Err())
		}
