...
func buildTestData(t *testing.T) *MemSeriesMetadata {
	...
	// Metrics.
	mem.Set("http_requests_total", 1, metadata.Metadata{Type: model.MetricTypeCounter, Help: "Total HTTP requests"})
	mem.Set("temperature", 2, metadata.Metadata{Type: model.MetricTypeGauge, Unit: "celsius"})
...
func TestWriteFileWithOptions_NamespaceRowGroups(t *testing.T) {
	...
	// We should see at least metric, resource_table, resource_mapping, scope_table, scope_mapping.
	require.True(t, seenNamespaces[NamespaceMetric])
	require.True(t, seenNamespaces[NamespaceResourceTable])
	require.True(t, seenNamespaces[NamespaceResourceMapping])
	require.True(t, seenNamespaces[NamespaceScopeTable])
	require.True(t, seenNamespaces[NamespaceScopeMapping])
}
...
func TestWriteFileWithOptions_RoundTrip(t *testing.T) {
	...
	meta, found = reader.GetByMetricName("temperature")
	require.True(t, found)
	require.Equal(t, "celsius", meta.Unit)
...
func TestReadSeriesMetadataFromReaderAt_NamespaceFilter_EmptyResult(t *testing.T) {
	tmpdir := t.TempDir()
	mem := NewMemSeriesMetadata()
	mem.Set("test_metric", 1, metadata.Metadata{Type: model.MetricTypeCounter})
