...
func (a *headAppender) Append(ref storage.SeriesRef, lset labels.Labels, t int64, v float64) (storage.SeriesRef, error) {
	return a.appendSample(ref, lset, t, v, a.getOrCreate)
}

func (a *headAppender) appendSample(ref storage.SeriesRef, lset labels.Labels, t int64, v float64, getOrCreate func(labels.Labels) (*memSeries, error)) (storage.SeriesRef, error) {
...
func (a *headAppender) getOrCreateInfoMetric(lset labels.Labels, t int64, identifyingLabels []int) (*memSeries, error) {
	fmt.Printf("headAppender: getOrCreateInfoMetric\n")
	s, err := a.getOrCreate(lset)
	if err != nil {
		return s, err
	}
...
func (a *headAppender) AppendInfoSample(ref storage.SeriesRef, lset labels.Labels, t int64, identifyingLabels []int) (storage.SeriesRef, error) {
	fmt.Printf("headAppender.AppendInfoSample\n")
	return a.appendSample(ref, lset, t, 1, func(lset labels.Labels) (*memSeries, error) {
		return a.getOrCreateInfoMetric(lset, t, identifyingLabels)
	})
...
func (a *headAppender) log() error {
	...
	if len(a.infoSamples) > 0 {
		...
		samples := make([]record.RefSample, 0, len(a.infoSamples))
		for _, s := range a.infoSamples {
...
func (a *headAppender) Commit() (err error) {
	...
	for i, s := range a.infoSamples {
		...
		oooSample, _, err := series.appendableInfoSample(s.T, series.lset, s.ILs, a.headMaxt, a.minValidTime, a.oooTimeWindow)
		switch {
		case err == nil:
			// Do nothing.
		case errors.Is(err, storage.ErrOutOfOrderSample):
		...
		case oooSample:
			...
			*/
		default:
			ok, chunkCreated = series.appendInfoSample(s.T, s.ILs, a.appendID, appendChunkOpts)
			if ok {
				if s.T < inOrderMint {
					inOrderMint = s.T
				}
...
func (s *memSeries) insert(t int64, v float64, chunkDiskMapper chunkDiskMapper, oooCapMax int64) (inserted, chunkCreated bool, mmapRefs []chunks.ChunkDiskMapperRef) {
	if s.ooo == nil {
