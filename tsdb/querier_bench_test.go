...
func BenchmarkQuerierSelect(b *testing.B) {
	...
	block, err := OpenBlockWithOptions(nil, blockdir, nil, nil, seriesHashCache.GetBlockCacheProvider("test"), DefaultPostingsForMatchersCacheTTL, DefaultPostingsForMatchersCacheMaxItems, DefaultPostingsForMatchersCacheMaxBytes, DefaultPostingsForMatchersCacheForce, NewPostingsForMatchersCacheMetrics(nil), emptyStats{})
	require.NoError(b, err)
	defer func() {
		require.NoError(b, block.Close())
	}()
