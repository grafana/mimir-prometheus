...
func (s *stripeSeries) gc(mint int64, minOOOMmapRef chunks.ChunkDiskMapperRef) (_ map[storage.SeriesRef]struct{}, _ map[labels.Label]struct{}, _, _ int64, minMmapFile int, _ stripeSeriesStats) {
	...
	survivors := swiss.NewMap[chunks.HeadSeriesRef, *memSeries](uint32(prevSurvivors.Count()))
	...
	// Check survivors.
	prevSurvivors.Iter(func(_ chunks.HeadSeriesRef, series *memSeries) (stop bool) {
		series.Lock()
		defer series.Unlock()
