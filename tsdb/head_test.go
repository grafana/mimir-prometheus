...
func TestHead_mmapHeadChunks(t *testing.T) {
	...
	for _, lbls := range []labels.Labels{lblsB, lblsC} {
		...
		s.Lock()
		headChunks := s.headChunks
		var previousHeadChunk *memChunk
		if headChunks != nil {
			previousHeadChunk = headChunks.prev
		}
		mmappedChunkCount := len(s.mmappedChunks)
		s.Unlock()

		require.NotNil(t, headChunks, "series %s should have head chunks", lbls)
		require.Nil(t, previousHeadChunk, "series %s should not have prev mmapped", lbls)
		require.Positive(t, mmappedChunkCount, "series %s should have mmapped chunks", lbls)
	}
...
func TestQuerierDoesNotBlockHeadTruncation(t *testing.T) {
	...
	assertDoesNotBlock := func(t *testing.T, db *DB, cl io.Closer) {
		...
		// The wait polls every 500ms, so this is long enough for it to either finish or block.
		<-time.After(550 * time.Millisecond)
