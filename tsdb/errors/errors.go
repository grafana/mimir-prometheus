...
// NB: this package was removed from prometheus/prometheus in https://github.com/prometheus/prometheus/pull/17768
// we're adding it back to allow downstream projects some time to migrate off of it.
...
func (es *multiError) Add(errs ...error) {
	for _, err := range errs {
		...
		if merr, ok := errors.AsType[nonNilMultiError](err); ok {
