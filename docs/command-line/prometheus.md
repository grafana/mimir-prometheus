...
| <code class="text-nowrap">--query.info-resource-strategy</code> | Strategy for resolving resource attributes in info(): 'target-info' uses target_info metric-join, 'resource-attributes' uses stored resource attributes (requires native-metadata), 'hybrid' combines both (requires native-metadata). Use with server mode only. | `target-info` |
